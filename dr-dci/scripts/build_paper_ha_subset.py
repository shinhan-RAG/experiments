"""paper-ha subset / 질의 표본 생성 (LLM 호출 없음). build_shinhan_subset.py 와 동일 계약.

Part 1·3은 config의 `subset`이 non-null이어야 augmentation이 적용된다.
`subset: null`이면 run_experiment.load_augmentations 가 조용히 None을 반환해
모든 arm이 baseline으로 붕괴한다. 코퍼스가 4,253청크뿐이어도 subset 파일을 만든다.

  subset_size 5000 -> size_key "5k"

출력:
  data/subsets/paper-ha/5k.json
  data/subsets/paper-ha/sampled_queries.json

주의: `data/raw/paper-ha/5k_parent_ids.json`은 만들지 않는다 (parent_id 없는
청크형 corpus에서 parent 분기를 타면 잘못 필터링된다).

  python scripts/build_paper_ha_subset.py
"""
import json
import sys
...
DATASET = "paper-ha"
SUBSET_SIZE = 5000  # -> "5k". 실제 4,253청크지만 size_key 규칙상 5000으로 올린다.
...
def build() -> None:
    ...
    with open(subset_path, "w", encoding="utf-8") as f:
        json.dump({
            ...
            # 코퍼스(4,253)가 subset_size(5000)보다 작아 서브셋이 짧다는 선언.
    ...
    if stale.exists():
        print(f"  [!] {stale} 가 존재한다. 삭제할 것 (parent 분기 오작동).")
