...
class DCIAgent:
    def __init__(self, llm_url: str, model_name: str, retriever: PullRetriever,
                 ...
                 api_key: str = None, taxonomy_boost_data: dict = None,
                 temperature: float = 0.0, llm_max_tokens: int = 1024,
                 llm_seed: int = None):
    ...
    def run(self, query: str) -> dict:
        ...
        return {
            ...
            "taxonomy_boost_eligible_documents": state["taxonomy_boost_eligible_documents"],
            "taxonomy_boosted_positive_score_documents": state["taxonomy_boosted_positive_score_documents"],
            "taxonomy_boosted_returned_documents": state["taxonomy_boosted_returned_documents"],
            "taxonomy_boost_rank_changed_pulls": state["taxonomy_boost_rank_changed_pulls"],
            "taxonomy_boost_top_k_entered_documents": state["taxonomy_boost_top_k_entered_documents"],
            "taxonomy_boost_top_k_exited_documents": state["taxonomy_boost_top_k_exited_documents"],
            "taxonomy_boost_target_score_count": state["taxonomy_boost_target_score_count"],
            "taxonomy_boost_target_negative_score_count": state["taxonomy_boost_target_negative_score_count"],
            "taxonomy_boost_target_score_min": state["taxonomy_boost_target_score_min"],
            "taxonomy_boost_target_score_max": state["taxonomy_boost_target_score_max"],
            "taxonomy_boost_telemetry_seconds": state["taxonomy_boost_telemetry_seconds"],
            "pull_queries": state["pull_queries"],
            "pull_traces": state["pull_traces"],
        ...
        if name == "pull":
            ...
            try:
                try:
                    ...
                    # Legacy test doubles and pre-P0 retrievers expose only
                    # the two semantic arguments.  Production retrievers must
                    # use the bounded/backfill API above.
                    if "unexpected keyword" not in str(exc):
                        raise
                    pulled = self.retriever.pull(
                        query=query, taxonomy_filter=args.get("taxonomy_filter")
                    )
            ...
            telemetry = getattr(pulled, "telemetry", {})
            eligible_ids = set()
            if args.get("taxonomy_filter") and self.taxonomy_boost_data:
                eligible_ids = {
                    doc_id for doc_id, taxonomy in self.taxonomy_boost_data.items()
                    if isinstance(taxonomy, dict)
                    and all(taxonomy.get(key) == value for key, value in args["taxonomy_filter"].items())
                }
            eligible = telemetry.get("taxonomy_boost_eligible_documents", len(eligible_ids))
            state["taxonomy_boost_eligible_documents"] += eligible
            state["taxonomy_boosted_positive_score_documents"] += telemetry.get("taxonomy_boosted_positive_score_documents", 0)
            state["taxonomy_boosted_returned_documents"] += telemetry.get(
                "taxonomy_boosted_returned_documents",
                sum(row["doc_id"] in eligible_ids for row in pulled["results"]),
            )
            state["taxonomy_boost_rank_changed_pulls"] += int(bool(telemetry.get("taxonomy_boost_rank_changed", False)))
            state["taxonomy_boost_top_k_entered_documents"] += telemetry.get("taxonomy_boost_top_k_entered_documents", 0)
            state["taxonomy_boost_top_k_exited_documents"] += telemetry.get("taxonomy_boost_top_k_exited_documents", 0)
            state["taxonomy_boost_target_score_count"] += telemetry.get("taxonomy_boost_target_score_count", 0)
            state["taxonomy_boost_target_negative_score_count"] += telemetry.get("taxonomy_boost_target_negative_score_count", 0)
            state["taxonomy_boost_telemetry_seconds"] += float(telemetry.get("taxonomy_boost_telemetry_seconds", 0.0) or 0.0)
            target_min = telemetry.get("taxonomy_boost_target_score_min")
            target_max = telemetry.get("taxonomy_boost_target_score_max")
            if target_min is not None:
                state["taxonomy_boost_target_score_min"] = target_min if state["taxonomy_boost_target_score_min"] is None else min(state["taxonomy_boost_target_score_min"], target_min)
            if target_max is not None:
                state["taxonomy_boost_target_score_max"] = target_max if state["taxonomy_boost_target_score_max"] is None else max(state["taxonomy_boost_target_score_max"], target_max)
            ...
            state["pull_traces"].append({
                "query": query,
                "taxonomy_filter": args.get("taxonomy_filter"),
                "taxonomy_boost_rank_changed": bool(telemetry.get("taxonomy_boost_rank_changed", False)),
                "taxonomy_boost_rank_changes": telemetry.get("taxonomy_boost_rank_changes", []),
                "workspace_document_ids_after": list(workspace.docs.keys()),
            })
