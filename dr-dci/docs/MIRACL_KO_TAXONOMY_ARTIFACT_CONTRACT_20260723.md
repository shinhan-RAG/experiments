...
Only `status=complete` with every planned batch recorded exactly once, per-batch retry counts within the approved cap, raw bytes matching the listed SHA-256, and rejoined assignments whose canonical SHA matches the receipt can form a manifest. `partial` and `failed` receipts are diagnostic records only. The full validator first verifies plan/approval/code/run bindings, raw bytes, and assignment rejoin, then mints an opaque `VerifiedTaxonomyGenerationReceipt`; `build_taxonomy_artifact_manifest()` accepts only that token, never a shape-only receipt object. The authorized audit verifies the same binding chain before comparing the result with the 110K artifact assignments. `--integrity-only` remains explicitly non-authorizing. No approval record is created in `config/` by this work. The fixture contains marked synthetic values which validate contracts only and cannot authorize execution.
...
- plan/run batch-control mismatch, per-batch retry-cap violation, raw-response byte tampering, cross-run receipt injection, duplicate successful-batch receipt entries, shape-only/partial/failed receipt manifest construction, approval-plan mismatch, and receipt/assignment mismatch;
