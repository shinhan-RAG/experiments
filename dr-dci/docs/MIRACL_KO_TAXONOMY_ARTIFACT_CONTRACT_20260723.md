...
The artifact manifest is not an approval record. Before any artifact exists, an artifact-free `TaxonomyGenerationPlan` binds MIRACL identity/unit, source scale 110K, four fixed input SHA-256 values, **`generator_source_commit`**, code-contract/aggregate SHA, generator/model/tokenizer/prompt/seed/parameters, title/text-only input contract, batch schema version, output artifact schema version, fixed projection method, determinism mode, and the canonical vLLM request-body template/hash. Its vLLM launch must contain exactly one positional model or `--model`, exactly one each of `--revision`, `--tokenizer`, and `--tokenizer-revision`, and at most one `--served-model-name`; those values are structurally extracted and must exactly equal the plan's model/tokenizer declarations. For `actual_execution`, both revisions must be lowercase 40-hex Hugging Face commit SHAs in generator metadata, runtime metadata, and launch arguments; branches, tags, and short SHAs fail. Model/tokenizer repository values must be remote Hub identifiers, not local snapshot paths. The canonical request body uses the served name when present, otherwise the approved model repository. Duplicate or conflicting launch forms fail. `--trust-remote-code` is prohibited because no separate immutable remote-code revision contract exists. `model_tokenizer_binding_kind=synthetic_test` is allowed only for fixtures and cannot pass actual-generation preflight or mint a verified receipt. The plan cannot contain an artifact body or artifact hash.
...
`validate_taxonomy_generation_preflight()` independently reads Git HEAD and porcelain from `--generator-source-root`, requires that checkout to be clean and equal to the planned source commit, then rechecks its code bytes. It does not inspect control-path dirtiness and does not require an artifact manifest. The plan also fixes `run_controls`: batch size, corpus-ID-sorted contiguous grouping, ordinal batch order, timeout, retry cap, retry-cap semantics (`per_batch_successful_response_v1`), resume policy, and idempotency mode. Thus `max_retries` is the maximum retry count for each successful batch response; `batch_summary.retries` is the exact aggregate of those raw per-batch counts. A `TaxonomyGeneratorRun` with different controls cannot produce an authorized receipt.
...
Only `status=complete` with every planned batch recorded exactly once, per-batch retry counts within the approved cap, raw bytes matching the listed SHA-256, and rejoined assignments whose canonical SHA matches the receipt can form a manifest. `partial` and `failed` receipts are diagnostic records only. The full validator first verifies that the actual code-contract hash and aggregate code hash equal the plan, plan generator metadata, and approval record; it then verifies raw bytes, run binding, and assignment rejoin before minting an opaque `VerifiedTaxonomyGenerationReceipt`. The token includes code and generator-spec hashes. `build_taxonomy_artifact_manifest()` accepts only that token and rechecks the full artifact's generator provenance; it never accepts a shape-only receipt object. The authorized audit verifies the same binding chain before comparing the result with the 110K artifact assignments. `--integrity-only` remains explicitly non-authorizing. No approval record is created in `config/` by this work. The fixture contains marked synthetic values which validate contracts only; an explicit synthetic model/tokenizer binding is rejected from both actual-generation preflight and verified-receipt creation.
...
- plan/run batch-control mismatch, per-batch retry-cap violation, plan/actual foreign-code contract injection, raw-response byte tampering, cross-run receipt injection, duplicate successful-batch receipt entries, shape-only/partial/failed receipt manifest construction, approval-plan mismatch, artifact generator-provenance swap, and receipt/assignment mismatch;
- request-controls mode without `--generation-config vllm`, mismatched server generation-config launch source, omitted disabled-thinking request control, omitted JSON response schema, and container-identical request-contract drift;
- model/revision/tokenizer/tokenizer-revision mismatch between vLLM launch and generator spec, mutable revision (`main`, tag, or short SHA), local snapshot repository, `--trust-remote-code` without immutable code provenance, served-name/request-model mismatch, duplicate positional/`--model` or repeated identity options, and synthetic model/tokenizer binding used for actual authorization;
