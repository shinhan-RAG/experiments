...
class MiraclKoLexicalSmokeTests(unittest.TestCase):
    def test_config_rejects_non_passage_or_unapproved_backend(self):
        ...
        with self.assertRaisesRegex(ValueError, "Pyserini"):
            validate_lexical_smoke_config(wrong_backend)

        wrong_runtime = {**config, "runtime": {**config["runtime"], "dependency_mode": "full"}}
        with self.assertRaisesRegex(ValueError, "sparse minimal"):
