...
## 6. 에이전트 하네스 예산·도구 설정 — 최적값 탐색 (2026-08-18 추가)
S5(page 40 · preview 160 · search ≤20 · read ≤8 · submit ≤10)는 **이전 등록 결과와의 비교(재현 게이트)용 출발값**이며 최적이라는 근거는 없다
(이전 MANIFEST 스스로 "OLD/NEW 우열은 예산에 따라 부호 반전 — 미확정"). 따라서 예산·도구 설정은 실험으로 정한다.

| 요인 | 수준 | 측정 |
|---|---|---|
| page(검색 1회 노출 건수) | 20 / **40** / 80 | R@5·R@10 + 세션 토큰·비용 |
| preview 길이 | **160** / 300 | 〃 |
| search 상한 | 10 / **20** | 〃 + 실제 사용 횟수 분포 |
| read 상한 | 4 / **8** | 〃 |
| 라우터 | 규칙 / LLM qtags 단독 / union | 〃 |
| 에이전트 슬롯 지정(--contract 등) | off / **on** | 〃 |
| 태그 헤더 노출 | **off** / on (A5) | 〃 |

절차: 층화 파일럿 60문항 × reps 2(sonnet)에서 **1요인씩** 변경(굵게 = 기준). 판정은 R@5 (McNemar, 문항 reps 평균) 와 비용의 Pareto — 같은 R@5면 토큰 적은 쪽.
파일럿에서 방향이 잡힌 요인만 전수 338 로 확정. 전 arm 결과·비용 기록(격자 최고값만 보고 금지).
