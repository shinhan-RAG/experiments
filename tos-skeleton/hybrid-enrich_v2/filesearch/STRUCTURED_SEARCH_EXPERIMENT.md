...
## batch 19 (내용 동등 감사) → Gold v22 — R@5 .9147 (2026-08-25)

명세 v1 §3의 "사람이 OR member 로 명시 등록" 조항에 따라 잔여 실패 26문항을 내용 동등 기준
(그 span 하나로 공식 답을 쓸 수 있는가)으로 표적 재감사했다. 1차(Opus) pass17/fix9,
교차검토(Sonnet)가 fix 전건·pass 전건을 원문 재검증하고 **0170 의 기존 결함 member 를 추가
발견·제거**(span 이 다른 특약 정의로 끊겨 90일 claim 미지지 — pass→fix, 불일치 1/26).
adjudication 후 fix 10 / pass 16 / exclude 0. 확장 억제 사례 다수(반대 취지 갱신형, 정액
100% 판본, 지시문뿐인 부표)를 기록 — 후하게 만들기 위한 확장이 아님을 유지.

부수 수리: v21 spec3 확장 member 에 jo 필드 누락 → 스크립트 수정 후 v21 재생성(스팬 동일).

**Gold v22**(sha f75805c8…, 213 전 문항 reviewed). opus full 제출 재채점:

| gold | R@1 | R@5 | R@10 | suff@5 | suff@10 | 0점 |
|---|---:|---:|---:|---:|---:|---:|
| v21 | .5951 | .9030 | .9229 | .8897 | .9131 | 14 |
| **v22** | .6025 | **.9147** | .9300 | **.9014** | .9178 | 12 |

R@5 CI95 [.8779, .9464]. single .9408 / multi .8175. 회수: 0177(판본 OR)·0055(본문 소멸조)·
0179(본문 세부규정)·0491(본문 제22조). 계보: v21→v22 는 명세 §3 사람 등록 기반 채점 정합화.

### .95 대비 잔여 −.0353 의 성분 (0점 12 + 부분)
0점 12 중 경성 MISS 6(0232·0009·0086·0490·0532 + α — batch19 에서도 "추가 동등 근거 없음 pass"
로 재확인) ≈ −.028 이 구조적 하한. 나머지는 선택 플리커·multi 꼬리. **현 스택·현 gold 정의에서
재현 가능한 상한은 ~.96, 실측 도달권은 .91~.93** 으로 판단한다.

## 독립 리뷰 후속 3건 (사용자 지시, 2026-08-24)

.9147 확정 후 독립 리뷰어(신선 컨텍스트, 실패 트레이스 직접 열람)가 "소진은 시기상조" 판정과
함께 3개 수단을 지목 — 사용자 승인으로 전부 실행.

### R10: 호스트측 결정론 제출 재조합 (채택)
`recompose_submissions.py` — 에이전트 제출 top-5 절대 보존, 첫 검색 top-2 를 6위부터 보충
(append-only → R@5 강등 구조적 불가). 기존 opus full run 426셀 재채점만으로 전 문항 순손익 확정
(에이전트 재실행 0회). 사전 오프라인 탐색에서 top-5 앞쪽 삽입 변형들은 기존 정답 밀어내기로
순손실(anchor2@45 −.0246, anchor3@345 −.0587) — 리뷰어가 경고한 제로섬 그대로. anchor2@append 만
무해: **R@5 .9147→.9171** (UP 1: 0400 .5→1.0, DOWN 0). R@10 .9300→.9370, R@1 +.0024.
산출물: `results_goldv22_recomposed_anchor2.jsonl`, `results_goldv23_recomposed_anchor2.jsonl`.

### Gold batch20: 0532·0490 표적 재감사 → v23 (정확성 개선, 점수 영향 0)
리뷰어 주장: 에이전트가 rank1 제출한 j00047(0532)·j00028(0490)이 gold 와 내용 동등인데 batch19
가 놓침. 검증: batch19 프롬프트의 "같은 bracket scope 안" sweep 제한을 전역 질문에 한해 문서
전체로 교정한 batch20 을 기존 2단 blind 채널로 실행(1차 opus + 독립 교차 sonnet, 완전 일치).
결과는 리뷰어 주장 **기각**: j00047 은 갱신·암보장개시일 규정 부재, j00028 은 서류 목록 구성
(4호 vs 5호, 진단서 세부요건) 실질 차이로 양 감사자 모두 배제. 대신 0532 에 갱신형 제2-1조
내용 동등 2건(j05644·j07027) 등록 → **v23**(sha 5f2404df…). 에이전트 미제출 조라 재채점 변화 0
— v23 은 정확성 개선으로만 기록. 0490 pass(3차 확인). 산출물: `audits/full_gold_audit_v1_batch20_*`.

### R9: reference_follow 게이트 완화 surface_or_routed_edge (진행)
0086 트레이스: U5 v14 edge_ledger 에 뇌혈관질환진단특약→j07559 hops=1 엣지가 있는데 질의 표면형
(표/코드/서류) 게이트 미충족으로 refollow `enabled:false`. `surface_or_routed_edge` 게이트 구현
(표면형 OR 라우팅된 특약 존재; 엣지 없으면 ranking 공집합 → fail-closed 유지), arm
`r9v_refgate_routed_edge_hybrid`. 게이트 절차: 213 페어 결정론 노출(check_exposure r1v vs r9v)
→ 발화 문항 무해성 → mechanism → full. (결과는 아래에 추가)

R9 결정론 게이트 결과(213 전수 페어, gold v22 기준 채점 스크립트/v23 동일): 노출(k=40) r1v 202
→ r9v 203, 이득 1(0086: 권외→27위, 표적 그대로), 손실 0, gold 첫 순위 하락 0, 추가 개선
1(0290: 34→29). 무해성: 첫 페이지 byte-동일 110/213(미발화), 상이 103/213 — 상이 문항에서도
gold 순위 하락 전무(밀려난 5칸은 전부 비-gold). R6 와 달리 경계 gold 밀어내기 없음.
남은 위험은 에이전트 선택 플리커(103문항 후보 구성 변화) → mechanism 페어로 확인:
`opus_r9v_mechanism12x2_goldv23_20260824` (0086·0290 + 변경페이지 표본 10, r1v vs r9v, 2reps).

R9 mechanism 판정(`opus_r9v_mechanism12x2_goldv23_20260824`, 0290 r9v rep0 인프라 오류 cell 은
동일 조건 재실행으로 교체 — `results_clean.jsonl`): r1v .8333 = r9v .8333, 문항별 차이 0/12.
표적 0086 은 27위 노출에도 에이전트가 미선택(양팔 0). **R9 기각** — 결정론 무해·노출 +1 이지만
에이전트 이득 0 으로 full 승격 요건 미달. 구현(`surface_or_routed_edge` 게이트·r9v arm)은
비활성 보존. 공식 arm 은 r1v 유지.

### 독립 리뷰 후속 3건 종합
| 수단 | 판정 | R@5 |
|---|---|---|
| R10 재조합(anchor2@append) | 채택 | .9147→**.9171** |
| batch20→gold v23 | 정확성 개선(점수 0) | .9171 유지 |
| R9 게이트 완화 | 기각(mechanism 동률) | — |

**공식 확정: r1v + gold v23 + R10 재조합, train 213 agent ITT R@5 = .9171**
(as-submitted .9147). 리뷰어가 지목한 나머지 후보(주계약 동등 등록)는 2단 blind 감사가 실질
차이로 기각 — "경성 MISS ≈ 구조 하한" 판정이 감사 3중 확인으로 강화됨. .95 는 현 스택·현
gold 정의에서 미도달로 종결하며, 잔여 수단은 스택 계열 교체(임베딩 후보 결합 확대 등) 수준.

## 메타 채널 소개문 ablation (사용자 질문, 2026-08-25)
"LLM 소개문 없이 규칙으로 대체 가능한가"를 메타 채널 단독 결정론 노출로 측정
(`vector_search/eval_view_ablation.py`, gold v23, 213문항, hybrid top-40 → JO 사상).
view 3안: V9(현행 LLM 소개문) / RAW(원문만) / RULE(U4 규칙 꼬리표를 소개문으로, LLM 0회).

| view | 노출@5 | @10 | @20 | @40 | MRR@40 |
|---|---:|---:|---:|---:|---:|
| V9 | .761 | .798 | .850 | **.892** | .534 |
| RAW | .700 | .770 | .859 | .873 | .536 |
| RULE | .756 | **.808** | **.869** | .873 | **.635** |

판정: ① 소개문 제거(RAW)는 @5 −6.1%p — 소개문 가치 실증. ② 규칙 대체(RULE)는 단독 기준
V9와 동급(@5)~우세(@10·MRR), 깊은 꼬리(@40)만 V9 우세. 문항별 교차(@5: RULE만 12 / V9만 13)
— 두 안이 서로 다른 문항을 맞힘. 캐비엇: 채널 단독 결정론 측정이며 전체 스택 agent ITT 아님;
RULE 은 태그 검색기와 동일 정보원이라 결합 시 상호보완성 감소 가능. 채택 변경 없음(V9 유지),
무-LLM 요구 시 RULE 이 검증된 대체안임을 등록.

### RULE 소개문 전체 스택 실측 (r11v, 사용자 지시)
arm `r11v_meta_rule_hybrid`(r1v 동일, meta_view=RULE) — opus 213×2 full
(`opus_r11v_metarule_full213x2_goldv23_20260825`). 오류 10셀 재실행 교체(2회), 0345 는 3회
연속 모델호출 API 실패(is_error, 0토큰)로 지속 실패 셀 문서화·ITT 0 보존(2셀).
사전 결정론 게이트: 첫página gold 노출 202→199 (이득 0086 / 손실 4건: V9 꼬리 노출).

| (gold v23, clean, R10 재조합) | R@1 | R@5 | R@10 | suff@5 |
|---|---:|---:|---:|---:|
| V9 (공식) | .6049 | **.9171** | .9370 | .9038 |
| RULE (r11v) | .6084 | .9041 | .9288 | .8873 |

페어: V9 우세 8문항 / RULE 우세 1 / 동률 204, 부호검정 p≈.039 — **유의한 V9 우세(−.0130)**.
채널 단독에서는 동급이던 RULE 이 전체 스택에서 지는 이유: RULE 소개문은 태그 검색기와 동일
정보원이라 결합 시 관점 다양성이 사라져, V9 가 다른 각도로 구해주던 문항(0231·0293 등)이
이탈. **판정: V9(LLM 소개문) 유지 확정. 무-LLM 전환 시 실측 비용 −1.3%p 를 수치로 등록.**

### 채택 변경 — 무-LLM 파이프라인 확정 (사용자 결정, 2026-08-25)
사용자 지시 "LLM 개입 없이 가는 걸로": −1.3%p 트레이드오프를 수용하고 **공식 arm 을
r11v_meta_rule_hybrid 로 전환**. 이로써 적재·색인·후보구성 전 파이프라인이 LLM 0회
(RULE 소개문은 `vector_search/build_view_rule.py` 규칙 생성 — 실측 사용본과 byte 동일 검증,
임베딩 manifest view_sha 754f9c76 일치). LLM 은 최종 선택 에이전트(opus)에만 존재.
**공식 확정: r11v + gold v23 + R10, train 213 agent ITT R@5 = .9041** (as-submitted .9041,
0345 지속 실패 2셀 ITT 0 포함). V9 자산은 대조·이력용 보존.

## RULE 스택 개선 라운드 1 — 독립 리뷰 2건 + 선검증 (2026-08-25)
리뷰어 A(메타 채널 렌즈)·B(전층 렌즈, 실패 A/C/D 분류) 병렬 실행. 오프라인/결정론 선검증 결과:

| 후보 | 판정 | 근거 |
|---|---|---|
| RULE+RAW 이중 view RRF | 기각 | 채널 단독 @5 .756→.746 — RAW 는 RULE 과 동일 원문 기반, 다양성 없음 |
| 형제 압축(sib_gap2, 번호 인접) | 기각 | r11v +.0059 이나 **V9 런 교차검증 −.0023** — 런 특이 규칙(승자의 저주) |
| 무가드 근사중복 축약(thr .85) | 기각 | V9 런 0109 손실 — 타 계열 동일취지 조가 gold 인 경우 제거됨 |
| **R11: 계열 가드 근사중복 축약** | **채택** | 같은 특약 계열(contract_core 동일) + bigram Jaccard≥0.75 + 질의 내 판본 변별 표면 시 보호. **양 독립 런 852셀 손실 0**: r11v .9041→.9112(2UP), r1v .9171→.9229(3UP). thr 스윕 .75~.85 안전, .70 절벽(양 런 손실) — .75 채택 |

R11 은 R10 과 같은 호스트측 결정론 재조합 층(순서: R11 dedup → R10 anchor).
근거: gold 명세 §3 이 같은 계열 내용 동등 판본을 OR 로 취급 → 판본 중복은 채점상 순수 슬롯 낭비.
`recompose_submissions.py --dedup-family-thr 0.75` 구현.

**공식 갱신: r11v + gold v23 + R11(0.75) + R10 = R@5 .9112** (suff@5 .8944, R@10 .9288).
산출물 `results_clean_r11r10_final.jsonl`. (참고: 보류 중인 V9 트랙도 동일 규칙으로 .9229.)

### 미실행 후보 (색인 변경 — 결정론 게이트 후 full 재실측 필요, 사용자 승인 대기)
1. **별표 존 소개문 오귀속 교정**(리뷰어 A#1): 권말 분류표 청크가 직전 특약 contract_key 를
   상속(0293 gold c05506~09 등 실측 확인) → 존 감지 + 표 제목 주입 + 본문 "부표N 참조" 역링크.
   실패 gold 다수가 별표라 기대효과 최대.
2. **subject_key 절단 조각 정제**(A#4): view 측 필터.
3. **제목·급부명 TITLE view 앙상블**(A#3): V9 [표현] 다양성의 문서 내부 대체.
4. **무공백 질의 분절 게이트**(B#2): 213 중 무공백 12문항 국소, greedy 최장일치(문서 내부 어휘).
5. C층(9~28위 노출-미제출)은 규칙 수단 없음(B 판정, R9 교훈과 일치).

## 개선 라운드 2 마감 + 일반화(G1·G2) (2026-08-25)

### 색인 후보 게이트 판정
- **RULE2(별표교정+subject정제 묶음) → 분해**: 채널 단독 @5 .756→.737 열세. 성분 분해 결과
  subject 정제(RULE2B)가 유해 성분 — **기각**. 별표 교정 단독(RULE2A)은 채널 @5 +0.5%p(표적
  0293 진입, 손실 0)이나 **전체 스택 노출 변화 0**(199=199, 순위 이동 0 — gold가 메타 채널
  18위라 상위10 병합에 미달) → **공식 view 교체 없음**, 코드·view 는 위생 개선으로 보존.
- **R12 무공백 분절**: 전체 스택 노출 199→200, 순위 상승 4건 — 그러나 이득 문항 전원이 이미
  실측 만점(0046·0074·0090·0100·0177)이고 유일 하락(0185: 8→19위)은 만점 문항 위험.
  기대이득 ≈0 + 하방 위험 → **기각**(R9 동형), 구현 비활성 보존.
- 라운드 1~2 통산 채택: **R11 하나** (.9041→.9112).

### G1: 일반화 리팩터 — 하드코딩 제거 (재실험 불필요 증명)
`doc_profile.py` 신설: 주계약 헤더·브랜드 노이즈 리터럴을 프로필로 분리(기본값=기존 리터럴),
`<doc>.profile.json` 사이드카 지원 + generic 폴백(첫 특약 이전 '상품명(무배당…)' 제목 줄에서
주계약 패턴 유도). `clm_search` 주계약 판정을 리터럴 startswith → '특약 미포함' 일반 규칙으로,
`build_view_rule` 별첨 존을 절대좌표(2,000,000) → 문서 길이 비율(0.6×doc_end)로.
**byte-동일 검증 4종 전부 통과**: 리팩터 전(git HEAD)/후 build_universe 산출물 byte 동일 ·
라우터 주계약 선택 집합 동일 · annex_start 불변(2471107) · view_RULE sha 불변(754f9c76).
→ 공식 .9112 는 수학적으로 불변, 전체 셋 재실측 불필요.

### G2: 문서 전이 스모크 — 260507판 (사용자 제공)
리팩터된 파이프라인을 **무수정**으로 260507 md(11M)에 적용: elements 43,500 / 조 10,163 /
계약 scope 181(주계약 1 정확 식별) / U4·U5 정상 생성 / audit_generic_schema 정렬 결함 0
(missing·extra tag 0), 축 커버리지 250212와 동등 수준(identity .999, function .75, locator .975).
generic 주계약 유도 폴백은 가상 상품명 합성 케이스로 단위 검증. **파이프라인 일반화 실증** —
단, 성능 전이는 260507 gold 부재로 미측정(하이퍼파라미터 전이 한계는 기존 caveat 유지).

## G3: 260507 성능 전이 파일럿 (사용자 지시 — gold v23 자동 이식 86문항)
이식: 조 단위 containment>=0.85 단일 매칭만(병합-창 제외) → 86/213 문항, manifest 계보 기록
(`gold_260507_pilot_v1.jsonl`, 편향: 미개정 문항 위주 — 보수적 1차 신호). 260507 전체 인프라
구축(태그 색인 + 메타 RULE view/임베딩, 코퍼스 경로 파라미터화 — 250212 회귀 byte-동일).

| 동일 86문항 | 결정론 노출@40 | agent ITT R@5 |
|---|---:|---:|
| 250212 (원 문서) | 95.3% | .9302 |
| 260507 (전이, 재튜닝 0) | 84.9% | **.733** (API지속실패 5 제외 시 .778) |

낙폭 −.20 (제외 기준 −.17). 확정치는 R11 미적용(as-submitted=R10) 기준. 원인 분해: ① 특약 개명·신설(CAR-T 등)로 질문 어휘-특약명 불일치
→ 라우팅 실패(A형 미노출이 0점의 다수) ② 조 분할 상이(7,612→10,163). ③ **R11 은 260507 에서
유해(−.047, 만점→0 다수)** — 잘게 쪼개진 같은 계열 고유사 조가 서로 다른 근거를 담아 "판본
중복=슬롯 낭비" 전제가 붕괴. R11 적용은 문서별 재검증 필수로 강등(250212 공식엔 유지).
④ 모델호출 API 지속 실패 5문항(0188·0208·0345·0409·0410 — 전 시도 동일 서명, 0345 는 250212
에서도 동일) — 검색기 무관, ITT 0 보존.
결론: **파이프라인은 전이되나 성능은 재튜닝 없이 .69~.78 수준** — 신문서 투입 시 (a) 특약명
동의 매핑(질문 어휘 대응) 재구축, (b) R11 등 후처리 재검증, (c) gold 감사가 필요함을 실측으로 등록.

### G3 후속: 특약명 동의 매핑(r15v) — 기각 (가설 반증)
가설 "미노출 10 = 특약 개명 탓" 검증. 양 판 목차 core 유사도>=0.5 규칙 매칭으로 개명 3쌍 추출
(3대→6대질병간병인, 일반암진단→암진단, 혈전용해→2대질병혈전용해), arm 전용 별칭 파일
(`aliases_260507.json`, `contract_alias_file` 옵션 — 공용 aliases.json 미오염)로 배선.
86문항 결정론 게이트: **노출 73=73, 순위 이동 0** — 개명 쌍은 코어 부분매칭이 이미 흡수 중이었음.
문항별 정밀 진단(트레이스·gold 대조)으로 확정한 미노출 10 의 실제 원인:
신설 특약 담보 재편 2(0041 소액암·0100 CAR-T — 질문-신상품 연결은 어휘 규칙 불가, QA 갱신 사안) ·
정확 라우팅 후 순위 미달 3(0179·0447·0478 — 260507 재튜닝 사안) · 부분매칭 오류 2(0278·0126) ·
전역/무공백 난문 2(0532·0134) · 이식 gold 품질 1(0306). 별칭 기제는 비활성 보존.

## G4: 260507 .90 도전 — 1단계 이식 gold 감사 (3배치 병렬)
86문항을 3배치로 나눠 primary(opus)×3 병렬 → validator → 교차검토(sonnet)×3 병렬 → adjudication.
일치율 85/86 (유일 불일치 0370 은 review 가 span 축소를 더한 상위호환 — review 채택).
판정: **fix 69 / pass 17** — 자동 이식 gold 의 80%가 수리 대상이었음. 결함 유형: 오매핑 4건
(0306·0332·0447·0478 — 무관 조 혼입), 부분지지 member 혼입(0532: 27→25개 완전근거로 전면 교체),
동등 조 대량 누락(대장점막내암 +49, 항암약물 정의 22, 납입면제 11판본, 상급종합병원 +10 등),
span 과대 다수, 개정 반영('중도부가특약' 삭제 확인, 8차→9차 분류).
산출물: `audits/transfer_gold_audit_260507_b{1,2,3}_*` 체인 + `out/gold_260507_pilot_v2.jsonl`
(sha 6da2864d, members 233→387).

**재채점 (기존 86×2 run, R10 적용·R11 미적용 — 에이전트 재실행 0회):**
| 기준 | v1 gold | **v2 gold(감사)** |
|---|---:|---:|
| ITT 86문항 | .733 | **.846** |
| API지속실패 5 제외 81문항 | .778 | **.898** |
flips UP 12 / DOWN 0. 낙폭 −.17 중 **+.12 가 gold 이식 결함**이었음 — "전이 낙폭의 주성분은
검색기가 아니라 측정(gold)"이 실증됨. 250212 의 "gold 매핑 문제" 교훈 재현.
잔여 0점 7(0041·0126·0134·0179·0278·0311·0532): 라우팅·순위 4 + 전역/무공백 난문 2 + 부분매칭 1.
.90 까지 잔여 −0.2%p — 2단계(라우팅·튜닝)로 도전 계속.

### 병렬 트랙 결산: 라우팅 개선 3변형 기각
r16v(rev_union)·r17v(+partial 0.5): 노출 73→70, 순위하락 14 — 역색인 병기가 quota 잠식(제로섬).
r18v(core-in-subject 오탐 가드): 73→72, 하락 6 — 가드가 정당 매칭도 억제. 구현 비활성 보존.

## G4 마감: 260507 .90 달성 — 확증 실측 (2026-08-25)
2단계(튜닝): 결정론 스윕 5변형(meta merge_k 15 / clm 22 / fact 12 / jo_top 8 / 결합) 전부 기각
— 4종 무효과, t4(jo_top 8)는 수혜 3문항이 기만점·20문항 순위 하락으로 기대값 음수.
**250212 튜닝값이 260507 에서도 국소 최적** — 설정 전이성의 긍정적 증거로 기록.

3단계(확증): 신규 86×2 opus run(`opus_pilot260507_goldv2_confirm_86x2_20260825`, seed 20260854,
gold v2 직접 채점). 오류 10셀 재실행 — 0188 회복(1.0), **0208·0345·0409·0410 양 rep 지속 실패**
(API is_error 서명, 검색기 무관), 0478 rep0 만 실패(rep1 유효). clean+R10:

| 기준 | R@5 | 참고 |
|---|---:|---|
| ITT 86문항(지속실패 0점 포함) | .8634 | 상한이 API 실패로 .953 에 눌림 |
| **지속실패 4 제외 82문항** | **.9055** | **.90 달성** (R@1 .7378, R@10 .9329, suff@5 .8547) |

계보: 이식 직후 .733 → gold 감사(v2) 재채점 .846/.898 → 확증 실측 .863/**.906**.
기여 분해: gold 감사 +.12 (전부), 라우팅·튜닝 0 (8변형 전부 게이트 기각).
...
## P1: LLM 태그 검색기(retriever_llm) 1차 실측 — 기각 (2026-08-26)
가설: 규칙 subject 절단 조각(6,895 element, 21%)을 LLM(haiku, 적재 1회·동결) 재태깅하면 규칙
정제(RULE2B) 실패 지점을 회복. 생성: 배치15·병렬4, 고유 텍스트 캐시(판본 중복 공유로 실호출
대폭 절감), 6,799 충전/96 폴백. 사고 2건 기록: 네트워크 단절 중 실패 응답이 캐시 오염(빈 결과
322건 제거 후 재개 — 생성기에 '실패는 캐시 금지' 반영 필요), 구세대 프로세스의 지연 종료가
정화본을 미정화 출력으로 덮어씀(345행 타입 위반 — sanitize 재조립으로 복구, sha 고정).

213 결정론 게이트(gold v23, r11v 기준선 199/213):
| 변형 | 노출 | 이득/손실 | 순위 ↑/↓ (top5/10 진입/이탈) |
|---|---:|---|---|
| r19v 대체 | 199 | +4/−4 (0532 권외→노출·0490·0072·0090 명중 / 0046·0059·0130·0234 상실) | 48/82 (16/34) |
| r20v 병기(∪) | 196 | +2/−5 | 32/59 (9/26) |

판정: **양 변형 기각.** 표적 명중(경성 난문 0532 노출 등)은 실증됐으나 광역 제로섬 교란이 압도
— 병기 실패로 원인이 '표면 소실'이 아니라 'subject 증가에 따른 슬롯 점수 인플레'로 확정.
LLM 태깅은 u2 시절·qtags 에 이어 3세대째 기각 — "구조화 태그의 LLM화는 결합층에서 진다,
LLM 이 이기는 형태는 새 문장을 쓰는 자산(소개문·gold 감사)뿐" 이 설계 원칙으로 굳어짐.
잔여 여지(미실행): LLM subject 에 별도 저가중 슬롯 부여 등 결합 재설계 — 평가셋 반복 튜닝
위험이 있어 새 근거 없이는 재개하지 않음. 산출물·게이트 로그 보존, retriever_llm 트랙 휴면.

### P1 기각 원인 디버깅 (2026-08-26)
통계: 교체 6,799 element 의 subject 수는 3.1→3.5 로 거의 불변 — '개수 인플레' 가설 기각.
실원인 2가지 확정:
1) **일반어 유입**: LLM subject 최고빈도가 보험료(910)·특약(398)·보험금(396)·암(299)·주계약(241)
   등 초고빈도 일반어 — 규칙은 이런 어휘를 subject 로 뽑지 않음(규칙 최고빈도는 구체어 위주).
   일반어 subject 1개 = 해당 어휘를 품은 모든 질문에서 수백 element 가 슬롯 점수 획득.
2) **주제성(aboutness) vs 등장(mention) 혼동**: 사례 0130("소액암진단특약 … 구비서류?" gold 5·6위
   → 권외). 라우팅은 양판 동일했으나, LLM 이 정의·검사방법 조각들에 '경계성종양·갑상선암' subject
   를 부여 → 질문의 질병명과 매칭된 동일 특약 정의 조각 수십 개가 gold(사고증명서 조)를 밀어냄.
   규칙 subject 의 암묵 스펙은 "그 대상을 규정하는 조각"(인용부호·정의문 패턴)이었는데 LLM 프롬프트
   는 "등장하는 대상어"를 뽑게 함 — 검색 의미론상 정밀도 하락.
재개 조건(P2)으로 등록: ① 프롬프트를 주제성 기준으로 재설계("이 조각이 규정/지급대상으로 삼는
것만, 언급 제외") ② 문서빈도 상한 필터(df>N 일반어 차단). 캐시 재사용 불가(재생성 필요)이므로
새 근거·승인 없이는 재개하지 않음.

## P2: 주제성 프롬프트 + df 필터 재생성 — 기각, LLM 태깅 트랙 종결 (2026-08-26)
생성기 v3(주제성 스펙 명시·범주어 금지·df>150 차단·실패 캐시 금지 — 단절 2회 무오염 통과),
6,594 충전/301 규칙 폴백. 213 게이트: 노출 199→197(이득 3: 0072 권외→4위·0490·0491 /
손실 5), 순위 ↑50 ↓69, top5/10 진입 10 vs 이탈 26 — **사전 고정 기준(이탈≤진입) 미충족, 기각.**
df 통계가 잔여 원인 노출: 해약환급금 안내 보일러플레이트 문단들이 여전히 문장 조각 subject 를
얻어 df 146~150 으로 상한 바로 아래 잔존. v1(대체)·union(병기)·v3(주제성) 3연속 게이트 기각으로
**결론 확정: 규칙 태깅의 '과소 태깅'은 결함이 아니라 보일러플레이트 억제 기능이었고, 절단 조각
element 군(21%)은 대부분 애초에 태그가 없어야 맞는 반복 안내문이다.** 표적 명중(0532·0072·0090
권외→상위 진입)은 3판 공통 재현 — 국소 가치는 실재하나 전역 결합 수단이 없음.
LLM 태깅 트랙 종결. 재개 조건: element 단위가 아닌 "질문군 표적" 결합(특정 질문 유형에만
LLM 태그 색인을 병행 조회) 같은 구조 변경 제안이 있을 때만.
