...
class SlotSearch:
    def __init__(self, elements_path, tags_path):
        ...
        try:
            _al = json.load(open(Path(__file__).resolve().parent.parent / "aliases.json", encoding="utf-8")); _al.pop("_comment", None)
    ...
    def ensure_structured(self):
        ...
        if not hasattr(self, "_structured"):
            ...
            self._structured = StructuredTagIndex(
                self.E, self.tags, Path(__file__).resolve().parent.parent / "aliases.json")
