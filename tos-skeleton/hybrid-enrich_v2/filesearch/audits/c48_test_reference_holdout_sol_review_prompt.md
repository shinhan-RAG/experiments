...
1. `filesearch/audits/c48_test_reference_holdout_qids.json`
2. `/Users/ralph/Desktop/신한라이프/data_set/noah_qaset/정답셋_149_test_v3_retrieval_250212.csv`
3. `filesearch/out/gold_test_source_to_u3jo_provisional_v1.jsonl`
4. `filesearch/out/elements_u3jo.jsonl`
5. `filesearch/audits/c48_test_reference_holdout_luna.json`

주의: 첫 감사의 적용 검증 오류가 있다: q0527 replace_group_members(group 0→j00022)는 기존 group 1의 j00022와 중복되어 validator가 거부한다. 최종 구조가 한 group이어야 하면 적용기가 group 삭제를 지원하지 않으므로 exclude해야 한다. 해당 판정을 복사하지 말고 현재 group/member와 원문을 다시 확인하라.
...
배치 qid 12개를 입력 배열 순서대로 정확히 모두 감사하라. 첫 감사 결과는 반박 가능한 참고 자료일 뿐이며, 각 qid마다 다음을 확인한다.
