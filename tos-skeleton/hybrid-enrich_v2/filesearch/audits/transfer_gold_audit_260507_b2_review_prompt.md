당신은 보험 약관 QA Gold의 retrieval-blind 독립 교차검토자다. 첫 감사자의 판정을 그대로 따르지 말고 260507 원문으로 각 결론(추가·교체·축소·배제)을 재검증하고, 동의하지 않으면 자신의 판정을 내라. 이번 대상은 **문서 개정판(260507)으로
자동 이식된 gold** 의 정확성·완전성 감사다.

허용 입력은 아래 다섯 파일뿐이다.

1. `audits/transfer_gold_audit_260507_b2_qids.json`
...
4. `out/jo_260507.jsonl`
5. `audits/transfer_gold_audit_260507_b2_primary.json` (첫 감사 결과 — 반박 가능한 참고 자료) (260507 조 단위 원문: element_id, title, text, char_start/char_end, contract_scope)
...
첫 감사의 대량 추가(수십 개 OR member)는 표본이 아니라 스팬·문언을 직접 확인해 과잉 등록(claim 미지지 조 혼입)이 없는지 특히 검증하라. 배치 qid 를 입력 배열 순서대로 정확히 모두 감사하라. 각 qid마다:
...
형식, replace_group_members 도 동일하되 그 group 의 member 전체를 대체, replace_span 은 op 상위 jo/c0/c1/evidence_role 에 교체값을 두고 members=[]. 좌표는 jo_260507.jsonl 의 해당 jo char_start <= c0 < c1 <= char_end 를 python 으로
