#!/usr/bin/env python3
"""Render a frozen retrieval-blind Gold audit prompt and hash manifest."""
...
OFFICIAL = "/Users/ralph/Desktop/신한라이프/data_set/noah_qaset/정답셋_348_train_v3_retrieval_250212.csv"
...
def render(qids, gold, jo, review_audit=None, known_error="", official=OFFICIAL):
    count = len(json.loads(Path(qids).read_text(encoding="utf-8")))
    review = bool(review_audit)
    role = ("당신은 보험 약관 QA Gold의 독립 교차검토자다. 첫 감사자의 판정을 그대로 따르지 "
            "말고 원문으로 각 결론을 재검증하라."
            if review else
            "당신은 보험 약관 QA Gold의 retrieval-blind completeness 감사 에이전트다.")
    files = [qids, official, gold, jo] + ([review_audit] if review else [])
    numbered = "\n".join(f"{index}. `{path}`" for index, path in enumerate(files, start=1))
    first_review = (
        "첫 감사 결과는 반박 가능한 참고 자료일 뿐이며, " if review else "")
    error_note = (f"\n\n주의: 첫 감사의 적용 검증 오류가 있다: {known_error}. "
                  "해당 판정을 복사하지 말고 현재 group/member와 원문을 다시 확인하라."
                  if known_error else "")
    return f"""{role}

허용 입력은 아래 {'다섯' if review else '네'} 파일뿐이다.
...
배치 qid {count}개를 입력 배열 순서대로 정확히 모두 감사하라. {first_review}각 qid마다 다음을 확인한다.
...
def main():
    ...
    parser.add_argument("--jo", default="filesearch/out/elements_u3jo.jsonl")
    parser.add_argument("--review-audit")
    parser.add_argument("--known-error", default="")
    parser.add_argument("--official", default=OFFICIAL,
                        help="official QA CSV for the selected split")
    parser.add_argument("--output", required=True)
    args = parser.parse_args()
    prompt = render(args.qids, args.gold, args.jo, args.review_audit,
                    args.known_error, args.official)
    output = Path(args.output)
    output.parent.mkdir(parents=True, exist_ok=True)
    output.write_text(prompt, encoding="utf-8")
    inputs = {"qids": args.qids, "gold": args.gold, "jo": args.jo,
              **({"review_audit": args.review_audit} if args.review_audit else {})}
    manifest = {
        "policy": "generated retrieval-blind audit prompt",
        "mode": "review" if args.review_audit else "primary",
        "inputs": {key: {"path": str(Path(path).resolve()), "sha256": sha256(path)}
                   for key, path in inputs.items()},
        "official_csv": {"path": str(Path(args.official).resolve()),
    ...
    print(json.dumps({"mode": manifest["mode"], "output_sha256": sha256(output)}, indent=2))
