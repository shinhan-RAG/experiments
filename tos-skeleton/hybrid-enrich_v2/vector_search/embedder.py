#!/usr/bin/env python3
"""임베딩 소스 추상화 — OpenAI 호환 /v1/embeddings 또는 로컬 sentence-transformers.

EMBED_ENDPOINT 가 있으면 원격(예: http://localhost:8101/v1), 없으면 로컬.
...
def _remote(texts, endpoint, batch=64):
    ...
    for i in range(0, len(texts), batch):
        ...
        req = urllib.request.Request(
            endpoint.rstrip("/") + "/embeddings", data=body, headers=headers)
...
def encode(texts, batch=32, max_len=1024, progress=False):
    ...
    if _local is None:
        ...
        _local = SentenceTransformer(MODEL, device=dev)
...
def source():
    return {
        "source": os.environ.get("EMBED_ENDPOINT") or "local-sentence-transformers",
        "model": MODEL,
    }
