#!/usr/bin/env python3
"""Run the 257-question C29 dual-tool evaluation with Claude Sonnet 5 via claude CLI."""
...
GOLD = FS / "out" / "gold_train_scoped_u3_reviewed_overlay_v1_clean257.jsonl"
QIDS = FS / "out" / "gold_train_scoped_u3_reviewed_overlay_v1_clean257_qids.json"

PROMPT = """아래 질문의 근거 조항을 검색하고 제출하십시오. 즉시 검색을 시작하십시오.
...
규칙:
1. search를 원 질문으로 최소 1회 실행하십시오 (C29 semantic-tag 파일서치).
2. msearch를 최소 1회 실행하십시오 (V9 BM25+dense 하이브리드 검색).
3. 유력 후보를 read로 검증하십시오. 여러 근거가 필요하면 모두 덮으십시오.
4. 관련도 순으로 최대 10개를 submit하십시오.
5. submit 성공 후 즉시 종료하십시오.
6. Gold, 평가 파일, 소스 코드를 열람하지 마십시오.
...
SYSTEM_PROMPT = (
    "You are an insurance document search agent. "
    "Use only the Bash tool to run python agent_tools.py commands. "
    "Start searching immediately without any conversation."
)
...
def claude_command(claude_bin: str) -> list[str]:
    return [
        ...
        "--allowedTools", "Bash(python agent_tools.py *)",
        "--output-format", "text",
    ]
...
def run_one(args, run_dir: Path, row: dict, rep: int, arm: dict) -> tuple[dict, dict]:
    ...
    started = time.time()
    meta = {"model": MODEL, "error": None}
    prompt_text = PROMPT.format(question=row["q"])
    prompt_file = session / "_prompt.txt"
    prompt_file.write_text(prompt_text, encoding="utf-8")
    try:
        with prompt_file.open("r", encoding="utf-8") as pf:
            proc = subprocess.run(
        ...
        output.write_text(proc.stdout[-8000:], encoding="utf-8")
        meta.update(returncode=proc.returncode, stdout=proc.stdout[-4000:], stderr=proc.stderr[-4000:])
        if proc.returncode:
            meta["error"] = f"claude_exit:{proc.returncode}"
    except subprocess.TimeoutExpired:
        meta["error"] = "timeout"
...
def main() -> None:
    ...
    parser.add_argument("--run", default="c29_dual_tool_clean257_sonnet5_w2")
    parser.add_argument("--workers", type=int, default=2)
    parser.add_argument("--reps", type=int, default=1)
    parser.add_argument("--n", type=int, default=-1, help="Smoke subset size; -1 uses all 257 questions")
    parser.add_argument("--seed", type=int, default=20260821)
    parser.add_argument("--timeout", type=int, default=900)
    parser.add_argument("--resume", action="store_true")
    parser.add_argument("--claude-bin", default=os.environ.get("CLAUDE_BIN") or shutil.which("claude") or "claude")
    ...
    (run_dir / "manifest.json").write_text(json.dumps(manifest, ensure_ascii=False, indent=2, default=str) + "\n", encoding="utf-8")
    print(f"run={args.run} n={len(rows)} reps={args.reps} model={MODEL} workers={args.workers} embed={embedding_endpoint}", flush=True)
    try:
