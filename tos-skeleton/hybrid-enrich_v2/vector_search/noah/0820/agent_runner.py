...
def build_extra(arm):
    ...
    if arm.get("active_prompt"):
        # A1: 기능 광고가 아닌 행동 지시(워크플로) — 0819 구축 효과와 구분되는 변인
        lines.append('   - submit 전에 search 와 msearch 를 각각 최소 1회 실행하십시오.')
        lines.append('   - 결과에 확신이 없으면 표현을 바꿔 재검색하십시오(고객 용어 ↔ 약관 용어).')
        lines.append('   - 서로 다른 조(jo)의 id 로 10개를 채워 제출하십시오.')
    # 주의: fallback_silent / submit_pad / submit_ref_merge / force_msearch 는 의도적으로 여기서
    # 아무 줄도 만들지 않는다(silent 원칙 — 0819 프롬프트 구축 효과 재발 방지).
    return ("\n".join(lines) + "\n") if lines else ""
...
def run_one(args, run_dir, g, rep, arm):
    ...
    cmd = [args.claude_bin, "-p", "--model", args.model, "--output-format", "json", "--max-turns", str(args.max_turns),
           "--allowedTools", f"Bash({args.pybin} agent_tools.py:*)",
           "--disallowedTools", "Read,Edit,Write,Grep,Glob,WebFetch,WebSearch,Agent,NotebookEdit,Task"]
    t0 = time.time()
    try:
        prompt_text = PROMPT.format(question=g["q"], vsearch=VSEARCH_DOC if arm.get("meta") else "", extra=build_extra(arm))
        if args.pybin != "python3":  # Windows 로컬 등 python3 alias 가 없는 환경
            prompt_text = prompt_text.replace("python3 agent_tools.py", f"{args.pybin} agent_tools.py")
        r = subprocess.run(cmd, input=prompt_text, capture_output=True, text=True, timeout=args.timeout, cwd=str(HERE), env=env, encoding="utf-8", errors="replace")
        try:
...
def main():
    ...
    ap.add_argument("--claude-bin", default="claude", help="claude CLI 경로(로컬 Windows 는 네이티브 exe 전체 경로)")
    ap.add_argument("--pybin", default="python3", help="에이전트가 도구를 부를 파이썬 명령(Windows 로컬: python)")
    a = ap.parse_args()
