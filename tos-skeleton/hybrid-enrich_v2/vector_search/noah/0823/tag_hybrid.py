#!/usr/bin/env python3
"""Semantic-tag retrieval: existing structured CLM + sparse BM25, fused by RRF.

The metadata V9 retriever is intentionally not imported here.  This module only
indexes semantic-tag records and their source elements. Semantic tags are not
embedded; vector retrieval remains exclusive to the frozen V9 metadata tool.
"""
...
def tokens(text: str) -> list[str]:
    """Stable Korean-friendly tokens plus compact character bigrams."""
...
def build_view(tag: dict, element: dict, text_chars: int = 800) -> str:
    ...
    fields = [
        ...
        "[태그검색문] " + str(tag.get("search_text") or ""),
        "[원문] " + " ".join((element.get("text") or "").split())[:text_chars],
    ]
...
def build_index(
    ...
    meta = {"n": len(ids), "tag_embedding": False, "channels": ["clm", "bm25"], "sources": payload["sources"]}
...
class TagHybridSearch:
    def __init__(self, elements_path: Path, tags_path: Path, jo_path: Path, index_dir: Path):
        ...
        self._route_cache = {}
        self._clm_cache = {}
        self._load_index()  # validate source hashes even for the CLM-only baseline arm
        ...
        if self._index is None:
            ...
            for key, path in current.items():
                if expected.get(key, {}).get("sha256") != sha256(path):
                    raise RuntimeError(f"태그 인덱스 소스가 변경되었습니다: {key}; 인덱스를 재생성하십시오")
        return self._index
...
def default_paths():
    return (
        FS / "out" / "elements_u2.jsonl",
        FS / "out" / "tags_u2_rules.jsonl",
        FS / "out" / "elements_u2jo.jsonl",
        HERE / "out" / "tag_index",
    )
