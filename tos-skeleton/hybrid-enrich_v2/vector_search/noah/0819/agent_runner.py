...
def run_one(args, run_dir, g, rep, arm):
    ...
    t0 = time.time()
    try:
        r = subprocess.run(cmd, input=PROMPT.format(question=g["q"], vsearch=VSEARCH_DOC if arm.get("meta") else "", extra=build_extra(arm)), capture_output=True, text=True, timeout=args.timeout, cwd=str(HERE), env=env)
        try:
