...
def tool_call(sess, qid, question, arm, action, value=""):
    ...
    proc = subprocess.run(command, cwd=str(HERE), env=env, capture_output=True, text=True,
...
def model_action(provider, model, reasoning_effort, prompt, output, timeout,
    ...
    if provider == "claude":
        claude_exe = os.environ.get(
            "CLAUDE_EXE",
            r"C:\Users\Atdev-pc\AppData\Roaming\npm\node_modules\@anthropic-ai\claude-code\bin\claude.exe")
        command = [claude_exe, "-p", "--model", model,
                   "--effort", reasoning_effort or "medium", "--tools", "",
                   "--system-prompt", sysp,
                   "--json-schema", SCHEMA.read_text(encoding="utf-8"),
                   "--output-format", "json", "--no-session-persistence",
                   "--max-budget-usd", str(max_call_budget_usd)]
        # Windows: the prompt goes over stdin, not argv - a positional arg here
        # can blow past cmd.exe's ~8K command-line limit once tool-call traces grow.
        proc = subprocess.run(command, input=prompt, text=True, capture_output=True, encoding="utf-8",
                              errors="replace", timeout=timeout, cwd=str(HERE), env=os.environ.copy())
        action = None
