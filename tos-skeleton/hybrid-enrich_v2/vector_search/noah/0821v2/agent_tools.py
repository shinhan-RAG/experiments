#!/usr/bin/env python3
"""Codex-facing search/read/submit CLI for the 0821v2 tag-hybrid experiment.
...
def main():
    ...
    if a.cmd == "search":
        ...
        output({
            "query": a.q,
            "channel": "tag:slot" if not (arm.get("tag_weights") or {}).get("sparse") else "tag:rrf",
            "slots_used": slots,
    ...
    if a.cmd == "msearch":
        if not arm.get("meta"):
            ...
            return
        effective_strategy = arm.get("meta_strategy", a.strategy)
        if arm.get("meta_strategy") and a.strategy != effective_strategy:
            output({"error": f"이 arm의 msearch 전략은 {effective_strategy}로 고정되어 있습니다."})
            return
        all_items = meta_items(a.q, PAGE * a.page, effective_strategy)
        items = all_items[PAGE * (a.page - 1) : PAGE * a.page]
        log({"qid": qid, "channel": f"meta:{effective_strategy}", "q": a.q, "strategy": effective_strategy, "page": a.page, "returned": [(x["id"], 0) for x in items]})
        output({
            "query": a.q,
            "channel": f"meta:{effective_strategy}",
            "page": a.page,
            "page_size": PAGE,
            "search_calls_left": SEARCH_CAP - n_search - 1,
            "results": items,
    ...
    successful_meta = any(
        call.get("cmd") == "msearch"
        and call.get("channel") == f"meta:{arm.get('meta_strategy', 'hybrid')}"
        and bool(call.get("returned"))
        for call in calls
    )
    if arm.get("require_meta_before_submit") and not successful_meta:
        log({"qid": qid, "ids": [], "rejected": "msearch_required"})
        output({"error": "제출 전에 결과가 있는 V9 hybrid 메타 벡터검색(msearch)을 최소 1회 성공시키십시오."})
        return
