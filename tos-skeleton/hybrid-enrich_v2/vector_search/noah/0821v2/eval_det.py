...
def main():
    ...
    if args.resume:
        ...
        for name in arm_names:
            ...
            rows = list(map(json.loads, path.open(encoding="utf-8"))) if path.exists() else []
            existing[name] = rows
            counts.append(len(rows))
        start_pos = min(counts) if counts else 0
        if any(count != start_pos for count in counts):
            raise SystemExit(f"resume arm counts mismatch: {counts}")
        if start_pos > len(gold):
            raise SystemExit(f"resume position exceeds gold: {start_pos}>{len(gold)}")
    handles = {name: (args.out_dir / f"{name}.jsonl").open("a" if args.resume and start_pos else "w", encoding="utf-8") for name in arm_names}
    values = {name: collections.defaultdict(list) for name in arm_names}
    for name, rows in existing.items():
        for row in rows[:start_pos]:
            for key in METRICS:
                values[name][key].append(row[key])
    try:
        for pos, g in enumerate(gold[start_pos:], start_pos + 1):
