#!/usr/bin/env python3
"""Run paired 4-arm agent evaluation on repaired Train-350 in 25-question batches."""
...
ARMS = {
    "BASE": ("vec_base", "grep_base_repaired_v1"),
    "OLD_META": ("vec_meta_v2", "grep_base_repaired_v1"),
    "META": ("vec_meta_v3_concat", "grep_base_repaired_v1"),
    "OLD_TAG": ("vec_base", "grep_tag_old_repaired_v1"),
    "TAG": ("vec_base", "grep_tag_new_repaired_v1"),
    "BOTH": ("vec_meta_v3_concat", "grep_tag_new_repaired_v1"),
}
...
class Search:
    def __init__(self):
        ...
        self.elements = {row["element_id"]: row["text"] for row in load_jsonl(OUT / "elements_repaired_v1.jsonl")}
        self.grep = {}
...
def first_rank(row, gold, spans):
    for rank, unit_id in enumerate(row.get("ranked_chunk_ids") or [], 1):
        span = spans.get(unit_id)
        if span and any(span[0] < end and span[1] > start for start, end in gold["gold_spans"]):
            return rank
    return None


def metrics(rows, gold_by, spans, arms):
    output = {}
    for arm in arms:
        arm_rows = {row["qid"]: row for row in rows if row["arm"] == arm}
        ranks = [first_rank(arm_rows.get(qid, {}), gold, spans) for qid, gold in gold_by.items()]
        n = len(ranks)
        output[arm] = {
            "n": n,
...
def print_table(done, result):
    print(f"\n== 누적 {done}/350 문항 ==", flush=True)
    print("Arm        R@1    R@5   R@10    MRR  err", flush=True)
    for arm, row in result.items():
        print(f"{arm:<9} {row['recall@1']:.3f}  {row['recall@5']:.3f}  {row['recall@10']:.3f}  {row['mrr@10']:.3f}  {row['errors']:>3}", flush=True)
...
def main():
    ...
    gold = load_jsonl(OUT / "train350_gold_repaired_v1.jsonl")
    if args.limit:
        gold = gold[:args.limit]
    output_dir = OUT / "agent_train350_repaired_sessions"
    ...
    results = []
    for batch_start in range(0, len(gold), args.batch_size):
        batch = gold[batch_start:batch_start + args.batch_size]
        jobs = [(row["qid"], row["question"], arm) for row in batch for arm in arms]
        with ThreadPoolExecutor(max_workers=args.workers) as pool:
            futures = [pool.submit(run_one, search, qid, question, arm, output_dir) for qid, question, arm in jobs]
            for future in as_completed(futures):
                results.append(future.result())
        done = batch_start + len(batch)
        paired = {row["qid"]: row for row in gold[:done]}
        summary = metrics(results, paired, spans, arms)
        print_table(done, summary)
        (OUT / "agent_train350_repaired_progress.json").write_text(json.dumps({"done": done, "metrics": summary}, ensure_ascii=False, indent=2), encoding="utf-8")
    with (OUT / "agent_train350_repaired_results.jsonl").open("w", encoding="utf-8") as handle:
