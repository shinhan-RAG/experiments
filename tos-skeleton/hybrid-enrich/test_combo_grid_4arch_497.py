#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""결합 방법론 재검증 축소판: META+TAG 재료 고정, 아키텍처 4종만, 497건 전체 기준(자동미스 포함)."""
import json, re, math
...
class BM25:
    ...
    def rank(self, query, id_list, top_n=100, allowed_ids=None):
...
def cosine_rank(q_vec, id_list, matrix, top_n=100, allowed_idx=None):
    import numpy as np
...
def eval_method(name, rank_fn, items, n_total):
    ...
    for it in items:
        ...
        for k in (5, 10, 20, 50, 100):
            if rk and rk <= k:
                hit[k] += 1
    return {"method": name, "n": n_total, **{f"R@{k}": round(hit[k] / n_total, 4) for k in (5, 10, 20, 50, 100)}}


def main():
    elements = load_jsonl(f"{OUT}/elements_semtag.jsonl")
    all_items = load_jsonl(f"{OUT}/gold_mapped_noah_v3.jsonl")  # 497건 전부, gold 없는 건 자동미스
    ...
    print(f"elements={len(elements)} n_total(497건)={n_total}", flush=True)
    ...
    # META+TAG 재료: META 요약 + 원문(TAG 채널이 담는 원문) 결합
    meta_tag_texts = [(meta_rows[eid]["embedding_text"] or "") + "\n[원문]\n" + el_by_id[eid]["text"][:2000] for eid in id_list]
    mat = embedder.encode(meta_tag_texts)
    print("META+TAG 임베딩 완료", flush=True)

    # 결합임베딩(concat)용: TAG 자체 표현 + META + 원문 전부 한 벡터로
    concat_all_texts = [
    ...
    results = []
    results.append(eval_method("점수융합(RRF) - META+TAG",
        lambda q: arch_rrf(bm_tag, q, id_list, qv(q), mat), all_items, n_total))
    results.append(eval_method("결합임베딩(concat) - TAG+META",
        lambda q: arch_concat(qv(q), id_list, concat_mat), all_items, n_total))
    results.append(eval_method("질의유형라우팅 - META+TAG",
        lambda q: arch_routing(bm_tag, q, elements, scope_cores, id_list, qv(q), mat), all_items, n_total))
    results.append(eval_method("계층재순위(cascade) - META+TAG",
        lambda q: arch_cascade(q, elements, scope_cores, id_list, id_to_idx, qv(q), mat), all_items, n_total))
    ...
    json.dump(results, open(f"{OUT}/combo_grid_4arch_497.json", "w", encoding="utf-8"), ensure_ascii=False, indent=2)
    print("저장: out/combo_grid_4arch_497.json")
